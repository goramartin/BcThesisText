...
Přehození rolí si vysvětlujeme overheadem za množství vytvářených polí (tj. hodně alokací, málo přístupů), které se vyrovná použité indirekci.
...
Jediný bod, který si neumíme vysvětlit je na obrázku \ref{figure.skitterGroupByPar} dotazy G\_Q4/G\_Q5 a obrázku \ref{figure.amazonGroupByPar} dotazy G\_Q3/G\_Q6 (dolní mez). 
...
Z výsledků single thread řešení jsme viděli, že rozdíl daných módů je minimální.
Dané zhoršení si vysvětlujeme druhým krokem (mergem do paralelní mapy).
Half-Streamed řešení zde vytváří a překopírovává struktury, jež mají více položek (tj. kopíruje větší množství dat).
...
Nyní přejdeme k výsledkům dotazů nepoužívající agregační funkce, obrázky \ref{figure.amazonGroupByParNoAgg}, \ref{figure.webberkstanGroupByParNoAgg} a \ref{figure.skitterGroupByParNoAgg}.
Opět zde dominuje řešení Normal: LocalGroupByLocalTwoWayMerge, což nás ujišťuje v předpokladu, že značné zpomalení ostatních řešení je způsobeno synchronizací.
Pokud bychom porovnali výsledky s grafy s agregačními funkcemi zjistíme konzistenci mezi výsledky. 
Dokonce se zde objevuje i zmíněná anomálie Streamed řešení z obrázků \ref{figure.skitterGroupByPar} a \ref{figure.amazonGroupByPar}.
...
Pro budoucí práci by bylo vhodné udělat podrobnější testování pro rozličtné počty použitých vláken a sledovat skalabilitu daných řešení.
...
Dané situace jsme netestovali a tento výzkum není součástí zadání této práce, nicméně může se jednat o další důvod, který způsobuje zpomalení vylepšených řešení. 
