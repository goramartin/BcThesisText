...
Problem představuje Streamed řešení, jelikož k jednomu výsledku přistupuje osm vláken najednou a dochází k synchronizaci.
...
\begin{figure}[!htp]
    ...
    \begin{minipage}{0.45\textwidth}
        \centering
        \includegraphics[width=0.9\textwidth]{../img/skitterGroupByQ1ParNoMin.pdf} % first figure itself
        \caption{Doba vykonání dotazu G\_Q1 pro graf As-Skitter (sekce \ref{tab.grafBase}) společně se Streamed řešením pro dotaz select avg(y.PropOne) match (x) -> (y) -> (z);. Běh osmi vláken. Počet seskupovaných výsledků je 453674558.}
        \label{figure.skitterGQ1ParNoMin}
    \end{minipage}\hfill
\end{figure}

Výsledek experimentu je zobrazen na obrázku \ref{figure.skitterGQ1ParNoMin}. 
Dokázali jsme, že problematická část je funkce \verb+min+. 