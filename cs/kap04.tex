...
Stávající a vylepšené verze Group/Order by jsou značně ovlivněny vyhledáváním vzoru. Řešení Half-Streamed a Streamed řešení zpracovává výsledky v moment jejich nalezení, zatímco Normal řešení je zpracovává po dokončení celého hledání.
...
Vylepšená řešení při přichozím výsledku jej uloží do tabulky (stejné tabulky jako v řešení Normal) a následně vloží index výsledku v tabulce do indexovací struktury, tj. v našem případě $(a, b)$-strom\footnote{\citet{dataLecture}}, kde $b=2a$. 
Používáme $b=256$.
...
Je to protože daný strom podléhá režii za insert $\Theta(\log n \cdot (a/\log a))$\footnote{\citet[03. (a,b)-trees str. 6]{dataLecture}}, kdy dochází k častému alokování nových vrcholů a překopírovávání prvků při splitu.
...
Daná situace vede k vytváření posloupnosti stejných hodnot ve vrcholech a následné nemožnosti využít jejich podstromy. 
...
Z grafů vidíme, že třídění podle \verb+ID+ (O\_Q1 a O\_Q2) vůči Properties (O\_Q3 a O\_Q4) vede ke znatelnému overheadu.
...
Při příchozím výsledku se získá hodnota prvního klíče třídění a určí se jeho přihrádka, kde počet přihrádek je heuristicky zvolen jako $m=t^2$, kde $t=\#vláken$. 
...
Pro dotazy O\_Q1 a O\_Q2 vidíme u Streamed řešení mnohonásobný rozdíl vůči ostatním řešením, protože přihrádky jsou rozděleny na základě rozsahu typu klíče, tj. \verb+[Int32.MinValue, Int32.MaxValue]+.
Avšak, hodnoty třídění spadaní do rozsahu \verb+ID+ vrcholů grafu, což představuje rozsah $\approx [0,\#vrcholů]$. 
Rozsah hodnot třídění spadá vždy do jedné přihrádky a výsledná doba je rovna doby single thread řešení s overheadem za přistupovaný zámek.
U dotazů O\_Q3 a O\_Q4 je tříděno pomocí hodnot vygenerovaných náhodně spadající do celého rozsahu typu klíče a předčil všechna ostatní řešení. 

Half-Streamed řešení se přibližuje Normal řešení v prvních dvou dotazech a překonává jej v třetím i čtvrtém dotazu pro řešení používající ABTreeValueAccumulator.
...
V druhém kroku u daného Half-Streamed řešení dochází k mergování pouze akumulovaných skupin, což rapidně sníží počet porovnávání a tady odtud výhoda oproti Normal Merge sort řešení.
