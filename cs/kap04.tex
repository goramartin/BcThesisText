...
Otestujeme i samotné seskupování, a to na dotazech G\_Q2 až G\_Q7. V dotazech nahradíme Select část prvním klíčem Group by. 
...
U dotazů G\_Q2$'$, G\_Q5$'$ a G\_Q7$'$ (bez agregačních funkcí) nastávají situace, kdy Half-Streamed řešení je pomalejší než Normal.
A navíc u grafu Amazon0601 i G\_Q4$'$, kde stejná situace existuje pro dotazy s ag. funkcemi G\_Q4 a G\_Q5.
Situaci jsme neočekávali.
Výsvětlujeme si ji následovně.
Dané dotazy by měly vygenerovat malé množství skupin.
Half-Streamed řešení používá při zpracování výsledku položku tabulky \verb+temporaryRow+, do které přesouvá pointr na pole výsleků.
...
Při úspěšném vložení nastane navíc překopírování výsleků do pravé tabulky.
...
Největší skok pak právě nastává v dotazech G\_Q3 a G\_Q6, kdy se ukládají dvě proměnné a navíc zde existuje vícero skupin.
...
Přesněji u dotazů G\_Q3 a G\_Q6.
...
Na grafu Amazon0601, pak u  G\_Q3 a G\_Q6 dotazů je Streamed řešení pomalejší než Half-Streamed (List), protože také vytváří pole jako Bucket řešení (viz implementace TODO).
U dalších grafů je pak počet vytváření polí mnohonosábně menší než počet přístupů k nim.

Můžeme říct, že vylepšená řešení pro single thread Group by jsou výhodnější z hlediska rychlosti vykonávání než řešení Normal. 
...
for (int i = 0; i < 1_000_000; i++)
{
    ...
    (1) if (!map.TryGetValue(i, out int value)) map.Add(i, i);
    (2) var tmp = parMap.GetOrAdd(i, i);
}
...
// Read.
...
\end{code}
Ukázalo se, že pouhé vkládání náhodně generovaných prvků do paralelní mapy trvá až 5-ti násobek doby obyčejné mapy.
Samostatné čtení náhodně generovaných hodnot, které existují v mapě, je o 9\% pomalejší.
Nicméně, v porovnání s řešením Normal: Global (použivá stejný princip) je vidět zrychlení, protože nepouživá tabulku výsledků.
Jediný bod, který si neumíme vysvětlit je na grafech \ref{figure.skitterGroupByPar} a \ref{figure.skitterGroupByParNoAgg} dotazy G\_Q4/G\_Q5 a obrázku \ref{figure.amazonGroupByPar} dotazy G\_Q3/G\_Q6 (dolní mez). 
Streamed řešení je pomalejší, než Normal: Global.
Všem řešením dominuje Normal: LocalGroupByLocalTwoWayMerge, které provádí vše lokálně a ujišťuje nás v předpokladu, že hlavní overhead je způsoben synchonizací.
...
Zpomalení Normal: Two-Step je ještě znatelnější pro dotazy G\_Q3/G\_Q3$'$ a G\_Q6/G\_Q6$'$, kdy se vytváří množství skupin.

Dále zde opět vidíme zpomalení implementace List vůči Bucket.
Rozdíly jsou znatelnější než u single thread řešení.
Neočekávané výsledky nastaly u řešení Normal: Two-Step proti Half-Streamed: Two-Step.
Half-Streamed řešení je pomalejší než Normal řešení nebo jsou vyrovnané.
U single thread řešení jsme viděli, že daná situace nastává jen u určitých dotazů bez agregačních funkcí a na nejmenším grafu Amazon0601 i pro dotazy s nimi.
Nyní nastává i u dotazů s ag. funkcemi.
...
Zvýhodnění nastává pouze v případě single thread vykonávání u Streamed řešení, jež nepoužívá tabulku výsledků vyhledávání.
...
Vylepšená řešení pak závisí na vlastnostech prohledávání grafu a může způsobit značné nerovnoměrnosti počtu výsledků mezi vlákny.
Situace je částečně řešena rozdělováním malých částí vrcholů k prohledání, ale nemůže zaručit stoprocentně rovnoměrné rozdělení.
