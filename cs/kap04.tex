...
Dané omezení pouze ovlivňuje pouze část \textit{Match}, což není určující pro náš experiment. 
...
Generování souborů \textit{Nodes.txt}/\textit{Edges.txt} provádí program, který je obsahem přílohy zdrojových kódů \ref{prilohy.codetransform} v souboru \textit{GrapDataBuilder.cs}.
...
Program je obsažen v příloze zdrojových kódů \ref{prilohy.codetransform} v souboru \textit{PropertyGenerator.cs}.
...
Tedy vrcholy představující proměnnou \texttt{x} jsou unikátní pro každé vlákno, taktéž hodnoty jejich vlastností.
...
\item G\_Q2 a G\_Q3  testuje vytváření skupin podle \verb+ID+ vrcholů. Rozdíl mezi nimi je ten, že proměnná \texttt{x} je při paralelním zpracování přístupná pouze jednomu vláknu za celý běh vyhledávání. Maximální počet skupin je shora omezen počtem vrcholů v~grafu.
...
Argumenty \verb+FixedArraySize+ a \verb+VerticesPerThread+ (sekce \ref{impl.arguments}) jsme zvolili pro experiment následovně:
...
V této sekci prezentujeme naměřená data pro všechny tři grafy (\ref{expr.graphs}), které jsme podrobili dotazům ze sekce \ref{expr.dotazy}.
...
Vylepšená řešení zpracovávají vyhledané výsledky v momentě jejich nalezení.
...
Dle našich předpokladů se ukázalo, že třídění pomocí hodnot vlastnosti (dotazy O\_Q3 a O\_Q4) vůči \verb+ID+ (dotazy O\_Q1 a O\_Q2) vede ke znatelnému zpomalení.
Při čtení hodnoty vlastnosti elementu grafu se přistupuje k databázi.
Během přístupu se ověřuje, zda daná vlastnost existuje na daném elementu.
Následně se hodnota načte.
Hodnota \texttt{ID} je uložena přímo ve struktuře elementu.
Hodnota tedy vždy existuje a nemusí se přistupovat k úložišti hodnot vlastností.
...
Vlákno v momentě nalezení výsledku přistupuje k přihrádce pomocí zámku a vloží do ní daný výsledek.
...
Ale problém je, že hodnoty \texttt{ID} vrcholů jsou z rozsahu $[0; \#$vrcholů v~grafu$]$, zatímco pouze jedna přihrádka obsahuje posloupnost 67 108 863 hodnot.
...
Z grafů \ref{figure.amazonGQ1Par}, \ref{figure.webberkstanGQ1Par} a \ref{figure.skitterGQ1Par} (paralelní \textbf{Streamed} řešení) v sekci \ref{expr.results.groupby.sinleGroup} je pak vidět značnou režii za synchronizaci thread-safe agregačních funkcí při přístupu osmi vláken.
...
Následuje příklad kódu použitého při testu (kód označíme \textit{InsertReadCode}):
\begin{code}
...
Měřeno dle kódu \textit{InsertReadCode} výše. 
Zkratka \textbf{Dict} označuje \texttt{Dictionary} a~zkratka \textbf{ConDict} označuje \texttt{ConcurrentDictionary}. 
...
Děje se tak na dvou grafech.
První graf je graf As-Skitter u dotazů G\_3/G\_3$'$ a G\_6/G\_6$'$.
...
V řešení \textbf{Half-Streamed} vlákna provádí stejnou práci jako v jednovláknovém zpracování.
Po dokončení prohledávání dojde ke sloučení všech lokálních výsledků.
V \textbf{Streamed} řešení vlákna pracují se sdíleným úložištěm výsledků a používá thread-safe funkce k aktualizaci výsledků.
...
Použijeme-li poznatky ze sekce \ref{matchResults} o zpomalení způsobeném ukládáním výsledků do tabulky zjistíme (v~našem případě jedné proměnné), že rozdíl mezi \textbf{Normal} a \textbf{Half-Streamed} řešením se pohybuje právě v rozsahu onoho zpomalení.
