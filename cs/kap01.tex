...
Nyní krátce popíšeme omezení a klíčové vlastnosti, s kterými budeme pracovat v naši práci.
...
Select část vytváří tabulku se dvěma sloupečky \text{x} a \texttt{y}, které obsahují \texttt{ID} elementů v daných proměnných.
...
Select část vytváří tabulku se dvěma sloupečky \text{x.VlasnostJedna} a \texttt{y.VlastnostDva}, které obsahují hodnoty vlastností elementů v daných proměnných.
...
Select část může obsahovat pouze výše zmíněné výrazy oddělené čárkami.
...
Order by definuje klíče (výrazy), podle kterých budou výsledky setříděny.
