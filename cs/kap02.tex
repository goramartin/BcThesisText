...
Tímto můžeme implementovat thread-safe funkci s pomocí atomických operací, jako funkce \texttt{Sum/Count}.
\end{itemize}
...
Všimněme si k nutnosti dvojité synchronizace.
...
To se dá implementovat například semaforem povolující vstup určitému počtu vláken.
Vlákno na přístup se pokusí projít skrze semafor, pokud je mu vstup zabráňěn dochází k rozšiřování tabulky.
...
Mínus je, že zde musí docházek mergování, ale v prvním kroku se využívají metody, které nemusí řešit synchoronizaci. 
Problematická část je zde mergování výsledků s List úložištěm výsledků agregačních funkcí.
První fáze nepedstavuje problém. 
V druhá představuje problém jako Global řešení.
Můžeme zkusit implementovat stejné řešení jako u Global pomocí semaforu.

\begin{figure}[!htp]
\includegraphics{../img/diaTwoGr.pdf}\centering
\caption{Diagram paralelizace Two-step Group by. Local part provádí ekvivalent single thread řešení. V Merge části dochází k mergování výsledků vláken pomocí paralelní mapy.}
...
K rozhodnutí, které finálně použít použijeme fakt, že počet výsledků k mergování je roven počtu vláken.
V takovém případě nebudeme implementovat paralelní mergování, ale pouze jedno vybrané vlákno provede sjednocení výsledků.
...
\section{Vylepšení částí Group/Order by}
