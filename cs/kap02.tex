...
Obecně k reprezentaci stromu výrazu potřebujeme $\mathcal{O}(n)$ paměti, kde $n$ je počet vrcholů stromu.
Pro výpočet výrazu potřebujeme projít skrze všechny vrcholy a na každém z nich trávíme konstantní čas, tj. celkově $\mathcal{O}(n)$.
Tímto jsme vyřešili problémy reprezentace výrazů pro náš dotazovací engine.
...
Tedy zaplněná tabulka společně s~úložišti hodnot agregačních funkcí zabere $\mathcal{O}(d\cdot (a_f+1))$, kde $d$ je počet výsledků prohledávání, $a_f$ je počet počítaných agregačních funkcí a $1$ je za ukládanou proxy třídu.
...
Výsledná paměťová složitost je $\mathcal{O}(a_f)$, kde $a_f$ je počet počítaných agregačních funkcí, protože pro každou funkci si musíme pamatovat její výsledek.
...
Vyhledávání běží v nejhorším případě jako $\mathcal{O}(\log n)$.
Prvek je následně vložen do pole.
Pokud se na místě nacházel již nějaký prvek, prvek je posunut doprava, což je $\mathcal{O}(n)$.
Pokud v poli není dostatek místa, tak je rozšířeno (pro nafukovací pole \citep[str. 211]{labyrint} je to $\mathcal{O}(1)$).
...
AVL stromy i Červeno-černé stromy vkládají prvek v~čase $\Theta(\log n)$ a mají prostorovou složitost $\mathcal{O}(n)$. 
...
Řešení obecně běží v čase $\mathcal{O}(d\cdot a_f)$, kde $d$ je počet výsledků prohledávání grafu. 
