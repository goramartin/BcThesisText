...
Pro paralelní zpracování jsme vymysleli tři přístupy: \textbf{Global}, \textbf{Two-step} a \textbf{Local + dvoucestný merge} (všechny budou schopny pracovat s úložišti Bucket i List).
...
V lokální části pro každé vlákno běží kompletně identický ekvivalent single thread řešení.
...
\subsection{Local + dvoucestný merge Group by}
...
Nevýhoda řešení je, že vlákna musejí čekat při mergi na dokončení práce druhého vlákna a teprve až pak může dojít k mergi.
...
\caption{Diagram paralelizace Local + dvoucestný merge Group by pro 4 vlákna.}
...
Čili, výsledky v průběhu hledání nebudeme ukládat do tabulky, která se po nalezení všech výsledků předá k dalšímu zpracování.
Namísto toho navrhneme postup, jak docílit zpracování výsledků v průběhu hledání.
...
V prvním kroku úprav musíme získat obecný pohled na pozměněné vykonání dotazu.
Následně v dalších krocích budeme navrhovat části dotazu konkrétněji.

\clearpage

\subsection{Pohled na pozměněné vykonání dotazu}

Analýzu začneme opět obecným pohledem na dotaz. 
...
Match část vygeneruje výsledek a ten předá k zatřídění části Order by nebo k seskupení části Group by.
Po zpracování se opět pokračuje v prohledávání grafu dokud se nenajde další výsledek a ten se zase předá další části.
...
V momentě nalezení se výsledek pošle dálší části.
...
Dotaz obsahující části Select, Match a Order/Group by bude reprezentovám opět řetězcem.
...
Objekty budou propojeny pomocí dvou úrovní.
První úroveň bude přímé spojení s objektem Match a druhá úroveň bude propojení vyhledávání a zpracování výsledku.
V této sekci popíšeme pouze první úroveň.
Vysvětlení druhé úrovně ponecháme na další sekci.
Abychom realizovali první úroveň, tak musíme upravit objekt Match.
...
Ještě než přistoupíme k druhé úrovni propojení zmíníme způsob volání funkcí \texttt{Compute}.
Prvě se zavolá na části Select, ta rekurzivně zavolá metodu na části Match a ta spustí vyhledávání.
Část Match prohledává graf a výsledky předává části Order/Group by. 
...
Tímto jsme dokončili popis první úrovně propojení.
Nyní přistoupíme k druhé úrovni.

\subsection{Propojení vyhledávání a zpracování výsledku}

První úroveň propojení pouze popisuje propojení objektů a komunikaci během vykonání dotazu.
Musíme ještě navrhnout způsob předávání výsledků částem Order/Group by v průběhu hledání.
U návrhu budeme vycházet přímo z původního návrhu z sekce \ref{anal.match}.
