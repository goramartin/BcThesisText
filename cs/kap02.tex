...
\end{code}
\textit{Match} vyprodukuje výsledky, Where je filtruje, \textit{Group by} je seskupí, Having opět filtruje, následně jsou setříděny a jako poslední krok se provede výpis uživateli.
...
Ačkoliv v naší práci použitá podmnožina PGQL (sekce \ref{req.pgql}) neobsahuje Having a Where, tak jsme si vědomi provázanosti částí \textit{Match/Where} a \textit{Group by/Having}.
...
Pro naše učely během implementace zkusíme zvolit prvně $n/\log_2 n$ ($n = $ \#výsledků prohledávání) pro jednovláknové zpracování a pro paralelní zpracování $(n/\log_2 n)/\#vláken$.
...
Global \textit{Group by} je znázorněn na obrázku \ref{figure.diaGlobalGr} na konci této sekce.
...
Two-step \textit{Group by} je znázorněn na obrázku \ref{figure.diaTwoGr} na konci této sekce.
...
Cílem úprav je poskytnout enginu schopnost provádět části \textit{Group by} a \textit{Order by} v průběhu prohledávání grafu části \textit{Match}.
...
Vycházíme z Global zpracování \textit{Group by} z sekce \ref{anal.groupby.global}.
...
Místo toho využijeme již několikrát zmiňované dvoucestné slévání (použili jsme jej při paralelizaci \textit{Group by} v sekci Local + dvoucestné slévání \ref{anal.groupby.local}).
...
Daný způsob nám příjde neefektivní, protože cílem \textit{Group by} je vytvořit skupiny. 
...
Při tomto řešení budeme vycházet z přístupu Global \textit{Group by} z sekce \ref{anal.groupby.global}, protože zpracovává výsledky stejným způsobem jako model \textbf{Streamed}.
...
Protože vycházíme z principu Global \textit{Group by} \ref{anal.groupby.global}, tak je zde problémem úložiště List.
