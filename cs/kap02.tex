...
\begin{code}
class ExpressionAdd<T>: ExpressionReturnValue<T> {
  public ExpressionReturnValue<T> expr1;
  public ExpressionReturnValue<T> expr2;
}

class ExpressionIntegerAdd: ExpressionAdd<int>{
  public override bool TryEvaluate(Element[] elms, out int retVal) {
...
Dotaz bude reprezentován třídou, která poskytne uživateli jednoduchý interface.
Interně bude třída obsahovat řetězec tříd, tj. exekuční plán.
Každá z těchto tříd obsahuje logiku a informace důležité pro výše zmiňované části dotazu.
Vykonání proběhne postupně dle priority.
Po dokončení zpracování se výsledky předají třídě další.    


\begin{figure}[!htp]
\includegraphics[width=\linewidth]{../img/classtest.pdf}\centering
...
\label{figure.amazonGroupByPareerere}
\end{figure}
