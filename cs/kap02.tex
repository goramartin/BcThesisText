...
Ať už budeme pracovat první nebo druhou variantou, musíme být schopni pracovat s řádky tabulky.
Order by řádky musí setřídit a Group by je seskupit.
Přesouvání řádku o více sloupcích je drahé.
Pro docílení efektivní práce s řádky budeme řádek reprezentovat proxy třídou.
Třída poskytne metody pro přístup k řádku a jeho hodnot ve sloupečcích.
Nesmí mít složité položky, aby bylo levné ji přesouvat.
V ideálním případě si bude pamatovat číslo řádku a odkaz na tabulku.
Nyní při Order by stačí pouze třídit proxy třídy. 

Pro výsledky Group by použijeme upravený princip.
Výsledky budou pravděpodovně zpracovány pomocí mapy/slovníku (C\# Dictionary).
Proxy třída nyní musí znát odkaz na klíč a hodnotu pro daný klíč.  
