...
Z tohoto důvodu nám vzniká hlavní omezení, které je paměťové, jelikož celá aplikace bude obsažena v hlavní paměti.
...
Expressions slouží k výpočtu hodnot ze zadaných výrazů.
...
Z kapitoly Předpoklady \ref{requirements} jsou hlavními faktory námi zvolená podmnožina jazyku PGQL a že se jedná o Property graf.
Pro případy nejednoznačnosti označíme \verb+elType+ jako typ štítku a \verb+propType+ jako skalární typ hodnoty vlastnosti.
...
V třídě by byl obsažen seznam \verb+ID+ elementů daného typu, jejich pořadí (např. dle vkládání do seznamu) a~vlastností v podobě polí s hodnotami.
...
Mějme $n$ elementů mající stejný \texttt{elType}.
Paměťová složitost struktury daného \texttt{elType} je $\mathcal{O}(n)$.
Pro každý element existuje jedna položka v seznamu \texttt{ID} a seznamu pořadí, tj. $\mathcal{O}(2\cdot n)$.
Navíc pro každý element existují položky v polích hodnot vlastností, což je $\mathcal{O}(n\cdot k)$, kde $k$ je počet vlastností daného \texttt{elType}.
Výsledně dostaneme $\mathcal{O}(2\cdot n+n\cdot k)$ a odtud lineární paměťová složitost.

\subsection{Struktury obsahující elementy} \label{anal.graphstructs}
...
Mějme graf $G=(V, E)$, kde $V$ je množina vrcholů a $E$ je množina hran.
Paměťová složitost výše popsaných polí pro $G$ je $\mathcal{O}(|V|+2\cdot |E|)$, protože existuje pole vrcholů, \texttt{in} hran a \texttt{out} hran.

Uvažovali jsme nad různými alternativami. 
Například mít jeden typ hrany obsahující všechny nutné informace.
Řešení je paměťově přijatelnější, ale nastává problém s přístupem k \texttt{in} hranám vrcholů.
Řešením může být vytvoření samostatného pole \texttt{out}/\texttt{in} hran pro každý vrchol. 
Daný přístup nám připadá náročnější z~hlediska paměti, protože musíme vytvářet pole pro každý vrchol zvlášť. 
...
51 EdgeY 0 1 44 // EdgeY má vlastnost Vlastnost1
                // s číselnou hodnotou.
52 EdgeX 1 2   
Soubor vrcholů (bez hlavičky a komentáře):
...
Nyní musíme analyzovat způsob získání informací z uživetelem zadaného dotazu. 
...
V naší představě bychom chtěli vygenerovat stromovou strukturu pro každou hlavní část dotazu (\textit{Match}, \textit{Select}, \textit{Order by} a \textit{Group by}).
...
Pokud nemají stejné jméno, je nutné určit vhodnou návratovou hodnotu.
...
Zbývá promyslet, jakým způsobem reprezentovat agregační funkce.
...
Na ukázce výše vidíme položku \verb+expr+, která reprezentuje vstupní výraz agregační funkce.
Funkce \verb+Apply+ je logikou funkce. 
Dále vidíme třídu \verb+Sum+, která představuje logiku funkce \texttt{sum}. 
Logikou funkce je přičtení vypočítané hodnoty do poskytnutého úložiště, pokud dojde k úspěšnému vyhodnocení výrazu.
...
\verb+accessedAggFuncID+ je \texttt{ID} vypočítané agregační funkce.
Hodnota funkce se vrací pomocí \verb+GetAggValue<T>+.
Obecně vždy nemusí dojít k úspěchu vyhodnocení.
...
Obecně k reprezentaci stromu výrazu potřebujeme $\mathcal{O}(v)$ paměti, kde $v$ je počet vrcholů stromu.
Pro výpočet výrazu potřebujeme projít skrze všechny vrcholy a~na každém z nich trávíme konstantní čas, tj. celkově $\mathcal{O}(v)$.
V našich vybraných případech (přístup k hodnotě \texttt{ID}/vlastnosti/výsledku agregační funkce a výpočet logiky agregační funkce) je $v=1$.
Výsledně při výpočtu $m$ výrazů pro $n$ prvků trávíme čas $\mathcal{O}(m\cdot n)$, což je lineární vzhledem k $n$.
...
Pokud bychom aplikovali první variantu, tak v~části \textit{Match} musíme vygenerovat $m + n$ sloupců hodnot.
...
Abychom zamezili stejnému problému také opačné strany (tj. mnoho proměnných a málo výrazů), tak budeme ukládat pouze proměnné, ke kterým se přistupuje v ostatních částech.
...
Ukládání výsledků do tabulky zabere $\mathcal{O}(d\cdot h)$ času (rozšíření tabulky amortizovaně v $\mathcal{O}(1)$ \citep[str. 211]{labyrint}), kde $d$ je počet nalezených výsledků při prohledávání grafu a~$h$~je počet sloupečků tabulky.
Výsledná tabulka pak zabere $\mathcal{O}(d\cdot h)$ paměti.
...
Hledání podgrafu v grafu je obecně NP složitý problém. 
...
K procházení grafu máme navrženou strukturu ze~sekce reprezentace grafu \ref{anal.grafrep}.
Pokud dojde k nalezení podgrafu, tak výsledek bude uložen způsobem ze sekce \ref{anal.tables}.
V našem případě tedy překopírováním elementů z pole proměnných náležící vzoru.

Mějme graf $G = (V, E)$, kde $V$ je množina vrcholů a $E$ je množina hran.
Uvažujme nyní, že máme vzor $z$, který obsahuje pouze jednu komponentu $k$.
Komponenta se skládá z $m_k$ hran a $n_k$ vrcholů.
Časová složitost prohledávání grafu pro vzor $z$ bude $\mathcal{O}(|V|\cdot |E|^{m_k})$, protože z každého vrcholu je spuštěno prohledávání a~z~prvního vrcholu se v nejhorším případě projde skrze všechny hrany.
Hrany mají vždy pouze jeden koncový vrchol a z daného vrcholu lze opět projít skrze všechny hrany.
Přidejme na konec vzoru $z$ druhou komponentu $l$. 
Vzor označme $z'$.
Komponenta $l$ má $m_l$ hran a $n_l$ vrcholů.
Uvažujme, že prohledáváním pro komponentu $k$ jsme nalezli $d_k$ výsledků.
Časová složitost prohledávání grafu pro vzor $z'$ bude $\mathcal{O}(|V|\cdot |E|^{m_k} + d\cdot |V|\cdot |E|^{m_l})$.
Prohledávání pro $k$ je rovnocenné a řekli jsme, že při nalezení jednoho výsledku první komponenty se provede prohledávání pro komponentu druhou.
Ekvivalentně pro více komponent.
Jsme si vědomi, že se jedná o~pomalý přístup, ale ušetříme tak spoustu paměti, protože nemusíme ukládat mezivýsledky.
Navíc typický případ je prohledávání pro jednu komponentu.
Pro experiment (kapitola \ref{expr}) použijeme vyhledávání pouze pro jednu komponentu.
Urychlení výpočtu skalárního součinu ponecháme jako budoucí rozšíření. 
Paměťová složitost samotného prohledávání je lineární vzhledem k délce vzoru, protože si musíme ukládat aktuálně procházené elementy.
Samotný graf je reprezentován strukturu ze sekce \ref{anal.graphstructs}, která zabere $\mathcal{O}(|V|+2\cdot |E|)$.
Ukládání výsledků do tabulky zabere $\mathcal{O}(d\cdot h)$ času, kde $d$ je počet nalezených výsledků při prohledávání grafu a~$h$~je počet sloupečků tabulky.
Výsledná tabulka pak zabere $\mathcal{O}(d\cdot h)$ paměti.
...
Další možná optimalizace porovnání může nastat v případě, pokud pro použitý graf platí \#Vrcholů $\ll $ \#Hran.
...
K tomu již máme navržené způsoby ze sekce Expressions (\ref{anal.expressions}).
...
Mějme $d$ výsledků prohledávání grafu a označme $a_{f}$ počet počítaných agregačních funkcí.
V~nejhorším případě bude každý výsledek reprezentován právě jednou skupinou.
Pro oba druhy úložišť platí, že zaberou v nejhorším případě $\mathcal{O}(d\cdot a_{f})$ paměti.
\clearpage
...
Každou funkci dokážeme spočítat v konstantním čase.
...
Z tohoto důvodu chceme využít strukturu, která bude interně používat hešovací tabulku (C\# \texttt{Dictionary<Key, Value>}).
...
Pro výsledek bude vypočítána heš na základě hodnot klíčů a následně použita při vložení do hešovací tabulky.
...
Uvažujme, že bychom finálně použili C\# \texttt{Dictionary} ke \textit{Group by}.
Daná hešovací tabulka je dle dokumentace implementovaná pomocí hešování s přihrádkami \citep[str. 268]{labyrint} a má přístup blízko $\mathcal{O}(1)$ v závislosti na použité hešovací funkci.
Jako hešovací funkci chceme zvolit nějakou hojně užívanou funkci\footnote{Příkladem mohou být funkce: \url{http://www.cse.yorku.ca/~oz/hash.html} [Dostupnost ověřena k datu 8.5.2021].}.
Iterujeme výsledky prohledávání grafu a seskupujeme je pomocí dané hešovací tabulky.
V nejhorším případě se vytváří v tabulce záznam pro každý výsledek prohledávání grafu.
Tedy zaplněná tabulka společně s~úložišti hodnot agregačních funkcí zabere $\mathcal{O}(d\cdot a_f+ d)$, kde $d$ je počet výsledků prohledávání a $a_f$ je počet počítaných agregačních funkcí.
$+d$ je za ukládanou proxy třídu řádků.
Celé jednovláknové seskupování výsledně běží v lineární paměťové i časové složitosti vzhledem k počtu výsledků prohledávání.

Pokud bychom uvažovali mód \textit{Single group Group by}, tak opět procházíme všechny výsledky prohledávání grafu, což nám dá lineární časovou složitost.
Nyní však existuje pouze jedna skupina, pro kterou počítáme agregační funkce.
Výsledná paměťová složitost je $\mathcal{O}(a_f)$, protože pro každou funkci si musíme pamatovat její výsledek.

\subsection{Optimalizace při výpočtu heš hodnoty} \label{anal.groupby.opt1}

Minulý přístup nám nabízí jednu optimalizaci k ušetření opakovaného výpočtu hodnoty klíčů.
Hešovací tabulka při vložení dvojice v prvním kroku vypočte hodnoty klíčů a vypočte jejich heš.
...
Zde můžeme využít předchozího výpočtu heš hodnoty.
Budeme uchovávat hodnoty výrazů a následně je znovu použijeme při porovnání.
Nicméně, pravděpodobně budeme potřebovat vytvořit závislost mezi objektem počítajícím heš hodnotu a objektem provádějícím porovnání.
...
Budeme vycházet ze sekce \ref{anal.vykonanidotazu}.
...
Propojení objektu \textit{Match} s novými objekty \textit{Group by} a \textit{Order by} navrhneme v~dalších sekcích práce.
...
V naší představě budeme vycházet přímo z původního návrhu ze sekce \ref{anal.match}.
...
Pokud bychom realizovali pouze jednovláknové vykonávání, tak bychom měli z~předchozích sekcí již kompletní návrh.
...
Vycházíme z \textbf{Global} zpracování \textit{Group by} ze sekce \ref{anal.groupby.global}.
...
Připomeneme, že ukládání výsledků do tabulky trvá $\mathcal{O}(d\cdot h)$, kde $d$ je počet nalezených výsledků při prohledávání grafu a~$h$~je počet sloupečků tabulky.
Výsledná tabulka pak zabere $\mathcal{O}(d\cdot h)$ paměti.
Při nalezení výsledku je výsledek vložen na nový řádek tabulky a index je zatříděn do indexační struktury.
...
Při zatřiďování indexu se využije binární vyhledávání \citep[str. 26]{labyrint}, které najde místo pro vložení indexu do pole.
Vyhledávání běží v nejhorším případě jako $\mathcal{O}(\log n)$, kde $n$ je počet prvků v poli.
Prvek je následně vložen do pole.
Pokud se na místě nacházel již nějaký prvek, prvek je posunut doprava, což je $\mathcal{O}(n)$.
Pokud v poli není dostatek místa, tak je rozšířeno (pro nafukovací pole \citep[str. 211]{labyrint} je to amortizovaně $\mathcal{O}(1)$).
Jedno zatřídění běží v nejhorším případě jako $\mathcal{O}(n)$.
Indexační struktura zabere celkově $\mathcal{O}(n)$ paměti. 
...
Chtěli bychom využít základní druhy vyhledávacích stromů.
Například vyvážené binární vyhledávací stromy (AVL stromy, Červeno-černé stromy) nebo $(a, b)$-stromy.
Připomeneme jejich základní složitosti.
AVL stromy i Červeno-černé stromy vkládají prvek v~čase $\Theta(\log n)$, kde $n$ je počet prvků ve stromě, a mají prostorovou složitost $\mathcal{O}(n)$. 
$(a, b)$-stromy vkládají prvek v~čase $\Theta(\log n)$ a mají opět $\mathcal{O}(n)$ prostorovou složitost.
...
\item \texttt{SortedSet}: reprezentuje vyvážený binární vyhledávací strom.
\end{enumerate}
...
Ukázalo se, že v průběhu testu \texttt{SortedSet} dosahoval využití paměti $6,8$\,GB, zatímco $(a, b)$-strom pouze $2,1$\,GB.
...
Nicméně, pole musíme občas rozšířit.
To lze amortizovaně v $\mathcal{O}(1)$ \citep[str. 211]{labyrint}.
...
Avšak, stále zde využijeme již navržené principy tabulek a stromů.
...
Výpočet správné přihrádky pro oba typy trvá konstantní dobu pomocí výše popsaných operací.
...
Opět budeme při zpracování chtít použít hešovací tabulku jako původní řešení.
...
V tomto módě chceme pouze upravit původní řešení \textbf{Two-step} \textit{Group by} ze~sekce \ref{anal.groupby.twostep}.
...
Sdílené úložiště je zde paralelní mapa/slovník (C\# \texttt{ConcurrentDictionary}).
...
K vytvoření skupin nám stačí znát pouze reprezentant skupiny, tj. klíč v hešovací tabulce (proxy třída řádku v tabulce výsledků prohledávání).
...
Pokud byla proxy třída použita k vytvoření nové skupiny, tj. je vytvořen nový záznam v hešovací tabulce, tak je celý výsledek držený v odkazu překopírován do tabulky.
...
Výsledně z pohledu jednovláknového řešení při seskupení trávíme čas $\mathcal{O}(d)$, kde $d$ je počet nalezených výsledků prohledávání grafu.
Prostorová složitost seskupení je $\mathcal{O}((s\cdot a_f+s)+(s\cdot k))$, kde $a_f$ je počet počítaných agregačních funkcí, $k$ je počet sloupečků tabulky výsledků prohledávání a $s$ je počet vytvořených skupin.
První závorka obsahuje využitou paměť hešovací tabulky a druhá obsahuje paměť tabulky výsledků prohledávání. 
V nejhorším případě je $s=d$, ale v typickém případě je $s\ll d$. Výsledně je využitá paměť lineární vzhledem k $s$.

\subsection{Řešení Streamed}

Při tomto řešení budeme vycházet z přístupu \textbf{Global} \textit{Group by} ze sekce \ref{anal.groupby.global}, protože zpracovává výsledky stejným způsobem jako model \textbf{Streamed}.
...
Vlákna v průběhu vyvolávají porovnaní klíčů v hešovací tabulce.
...
Výsledně z pohledu jednovláknového řešení při seskupení trávíme čas $\mathcal{O}(d)$, kde $d$ je počet nalezených výsledků prohledávání grafu.
Prostorová složitost seskupení je $\mathcal{O}(s\cdot a_f + s\cdot c)$, kde $a_f$ je počet počítaných agregačních funkcí, $c$~je počet klíčů seskupení a $s$ je počet vytvořených skupin.
První člen složitosti obsahuje paměť k~uložení výsledků agregačních funkcí a druhý člen obsahuje paměť k~uložení hodnot klíčů seskupení.
Vše je uloženo v hešovací tabulce. 
Jako u~\textbf{Half-Streamed} je v nejhorším případě $s=d$, ale v~typickém případě je $s\ll d$. Výsledně je využitá paměť lineární vzhledem k $s$.
...
Z pohledu jednovláknového řešení se využije $\mathcal{O}(a_f)$ paměti, kde $a_f$ je počet počítaných agregačních funkcí.
Řešení obecně běží v čase $\mathcal{O}(d)$, kde $d$ je počet výsledků prohledávání grafu. 
