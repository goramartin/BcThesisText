...
%%.2 DataFiles. 
%%.2 Parser.
%%.3 ParsedPattern \DTcomment{Definuje objekty syntaktického stromu posloupností části Match.}.
%%.3 Parser.
...
Třída postupně z tokenů vytváří syntaktické stromy každé hlavní části dotazu (Match, Select, Order by a Group by).
...
Každá část dotazu má svůj objekt implementující \texttt{IVisitor<T>}, například části Match přináleží objekt \texttt{MatchVisitor}.
...
V této sekci popíšeme implementaci části Match.
...
Třída dědí z \texttt{MatchObjectBase}, protože objekt části Match upraveného zpracování používá stejnou metodu ke kontrole vzoru uživatele (metoda \texttt{ParsedPatternCorrectness}).
...
Začneme popisem části Order by.
...
V této sekci popíšeme implementaci Group by.
...
Nyní pouze vytvoříme objekt \texttt{ResultProcessor} a nový objekt části Match \texttt{MatchObjectStreamed}. 
...
V této sekci popíšeme implementaci úprav části dotazu Order by.
...
V této sekci popíšeme implementaci úprav časti dotazu Group by.
...
V této sekci popíšeme implementaci úprav módu Single group Group by.
Při úpravě Single group Group by jsme postupovali přesně podle sekce analýzy \ref{anal.improvement.singlegroup}.
