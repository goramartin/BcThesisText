...
Určili jsme, že dotazovací engine bude pracovat v několika módech, které uživatel bude moct měnit.
Jsou to módy:
...
Z~dané knihovny jsme využili pouze určité algoritmy při implementaci části \textit{Order by}.
...
Samotnou implementaci algoritmů lze nalézt v odkazu citovaného zdroje. 
...
Níže popsané adresáře rozdělují dotazovací engine na hlavní části výstavby z kapitoly analýzy.
...
Každá sekce dokumentace začíná krátkým shrnutím pro lepší přehlednost. 
...
Při implementaci jsme postupovali dle návrhu ze sekce analýzy \ref{anal.grafrep}.
...
Což znamená, že v celém dotazovacím enginu je možno pracovat \textbf{pouze s těmito dvěma typy}.
...
\item \texttt{Dictionary<string, Tuple<int, Type> > labels} mapa, kde klíč je název vlastnosti a \texttt{int} je její přiřazený unikátní identifikátor, abychom nemuseli v dotazovacím enginu používat řetězce jako \texttt{ID} vlastnosti.
...
Při načítání dotazu jsme vycházeli ze sekce analýzy \ref{anal.parsing}.
...
Exekuční plán a samotné zpracování pak bude odpovídat popisu ze sekce analýzy \ref{anal.vykonanidotazu}.
...
Budeme vycházet ze sekce analýzy \ref{anal.match}.
...
Obsahuje indexy \texttt{int startVerticesIndex} a~\texttt{int startVerticesEndIndex}.
Dané dva indexy určují rozsah vrcholů z pole vrcholů \texttt{List<Vertex>}, ze kterých se bude spouštět prohledávání.
...
U implementace jsme vycházeli ze sekce analýzy \ref{anal.orderby}.
...
Potomci budou také implementovat zmíněné optimalizace ze sekce analýzy \ref{anal.orderby}.
...
Tím vyřešíme optimalizaci porovnání stejných elementů ze sekce \ref{anal.orderby.opt2}.
...
Tím vyřešíme optimalizaci porovnání stejných vlastností ze sekce \ref{anal.orderby.opt1}.
\end{enumerate}
...
Implementaci třídy jsme dále rozšířili dle návrhu porovnávání řádků tabulky při vkládání řádků do vyhledávacího stromu ze sekce analýzy \ref{anal.improvement.orderby.storeindex}.
...
U implementace \textit{Group by} jsme vycházeli ze sekce analýzy \ref{anal.groupby}.
...
Abychom pochopili implementaci seskupování musíme se podívat na způsob práce s hešováním výrazů.
V analýze jsme určili, že výsledky prohledávání budeme seskupovat pomocí hešovací tabulky, ať už v jednovláknovém nebo paralelním zpracování.
Jazyk C\# obsahuje hešovací tabulku \texttt{Dictionary<Key, Value>} pro jednovláknové zpracování a \texttt{ConcurrentDictionary<Key, Value>} pro paralelní zpracování.
...
To znamená, že se vypočte heš prvku a nalezne se místo vložení.
...
Při vrácení \texttt{false} dojde k vložení funkcí \texttt{Add}, která opět vypočte heš a případně porovná prvky.
...
Funkce \texttt{GetOrAdd} vypočte heš, získá místo vložení a pokud na místě jiný prvek není rovnou ho vloží.
Zde tedy dochází k hešování a porovnání pouze jednou.
V prvním případě \texttt{Dictionary} bychom byli nuceni vyhodnotit ten samý výraz 4-krát.
Obecně obě struktury pro dva rozdílné prvky se stejnou heš hodnotou mohou vytvářet spojový seznam daných prvků.
Výsledně bychom museli vypočítávat výrazy při každém porovnání v~seznamu.
Navíc, pokud by docházelo ke slévání výsledků dvou hešovacích tabulek do jedné, tak bychom opět museli počítat heš a porovnávat.

Obě struktury používají k výpočtu heš hodnoty a porovnání objekt s rozhraním \texttt{IEqualityComparer<T>}.
Kde \texttt{T} je \texttt{Key}. 
Objekt interně definuje dvě metody \\*\texttt{int GetHashCode(T o)} a \texttt{bool Equals(T x, T y)}.
První vypočte heš vkládaného objektu a při porovnání se vyvolá metoda \texttt{Equals}.
...
\item V momentě, kdy dochází k použití \texttt{Dictionary} nebo aktu slévání hešovacích tabulek, budeme jako \texttt{Key} používat strukturu \texttt{GroupDictKey}.
Struktura obsahuje dvě položky \texttt{int hash} a \texttt{int position}. 
\texttt{hash} je zde heš řádku a~\texttt{position} je index řádku v tabulce výsledků.
Tedy jsme rozšířili \texttt{Key} o~heš hodnotu řádku, kterou můžeme znovu použít, pokud to bude nutné.
Budeme implementovat vlastní \texttt{IEqualityComparer<T>}, který jen použije heš hodnotu ze struktury.
Tímto vypočteme heš pouze jednou za celý cyklus vkládání nebo slévání.
Pokud používáme \texttt{ConcurrentDictionary} a nedochází ke slévání, tak stačí jako \texttt{Key} volit index řádku.

\item Při výpočtu heš hodnoty se vypočítávají hodnoty výrazů.
Abychom nepočítali stejné výrazy opětovně při porovnání, tak budeme používat dvě třídy.
V prvním případě vytvoříme \texttt{ExpressionHasher}, který počítá heš hodnotu jednoho výrazu.
...
Toto odpovídá navržené optimalizaci ze sekce analýzy \ref{anal.groupby.opt1}.
...
\item Abstraktní metodu \texttt{int Hash(in RowProxy row)}, která vypočte heš výrazu pro řádek tabulky. 
...
Implementují výpočet výrazu pro konkrétní návratový typ \texttt{T} a výpočet heše výrazu.
...
Všechny klíče seskupení jsou obsaženy v~třídě \texttt{RowHasher}, která vypočte finální heš kombinací všech heš hodnot výrazů. 

\subsubsection{Rozhraní IEqualityComparer<T>}

V aplikaci jsme vytvořili dva objekty:
\begin{itemize}
\item \texttt{RowEqualityComparerInt: IEqualityComparer<int>} definuje porovnání a výpočet heš hodnot pomocí indexu řádku tabulky.
Třída drží odkaz na tabulku \texttt{TableResults} pro získání proxy třídy řádku indexem.
Dále má porovnávače \texttt{ExpressionComparer[]} pro zjištění rovnosti dvou prvků v hešovací tabulce a finálně \texttt{RowHasher} pro výpočet heše.
Položka \texttt{bool cacheResults} určuje zda se má použít druhá optimalizace.
\item \texttt{RowEqualityComparerGroupDictKey:} \\*\texttt{IEqualityComparer<GroupDictKey>}, který je totožný s předchozím, ale neobsahuje \texttt{RowHasher}, protože heš je uložen v objektu porovnání.
\end{itemize}
...
Potomci dané třídy jsou třídy \texttt{Min<T>} a~\texttt{Max<T>}.
...
Výsledně při používání úložiště Bucket vkládáme jako \texttt{value} do hešovací tabulky odkaz na pole vytvořené metodou \texttt{CreateBucketResult(..)}.
Při použití List nemůžeme použít odkazy na pole, protože hlavní myšlenka List je mít úložiště mimo hešovací tabulku, abychom nemuseli vytvářet spoustu tříd jako u Bucket.
...
Pole úložiště hodnot agregačních funkcí se vytváří pouze v momentě, kdy je vytvářen nový záznam do hešovací tabulky.
...
Řešení využívá \texttt{Dictionary<key, value>}, tudíž je zde opět \texttt{GroupDictKey} jako \texttt{key}.
Úložiště výsledků agregačních funkcí je vytvořeno na začátku v položce \texttt{aggResults}.
\texttt{value} je \texttt{int}, protože úložiště hodnot agregačních funkcí leží mimo hešovací tabulku.
Při přidávání nové skupiny do hešovací tabulky je určena nová pozice pro skupinu v úložišti \texttt{aggResults} pomocí \texttt{Dictionary.Count}.
Tato hodnota se vkládá s klíčem do hešovací tabulky a skrze ni se přistoupí k výsledkům funkcí pro skupinu.
\end{itemize}
...
\begin{itemize}
\item Řešení \textbf{Normal: Global (Bucket)} odpovídá paralelnímu zpracování ze sekce \ref{anal.groupby.global}.
Řešení je obsaženo v třídě \texttt{GlobalGroupByBucket}.
K seskupení vlákna využívají sdílenou hešovací tabulka \texttt{ConcurrentDictionary<key, value>}.
...
\item Řešení \textbf{Normal: Two-step (Bucket)} odpovídá paralelnímu zpracování ze~sekce \ref{anal.groupby.twostep} s úložištěm Bucket.
...
\item Řešení \textbf{Normal: Two-step (List) }odpovídá paralelnímu zpracování ze sekce \ref{anal.groupby.twostep} s úložištěm List.
...
\item \textbf{Normal: LocalGroupByLocalTwoWayMerge (Bucket)}\textbf{/(List)} odpovídají paralelním zpracováním ze sekce \ref{anal.groupby.local} s úložištěm Bucket/List.
...
Při slévání dvou tabulek dochází k překopírování záznamů z jedné hešovací tabulky do druhé.
...
\item \textbf{Normal: SingleGroup} odpovídájí módu \textit{Single Group Group by} zpracování ze sekce \ref{anal.groupby.singlegroup}.
...
Toto propojení vyplývá ze sekce \ref{anal.improvement.con2}.
...
První je obecný $(a, b)$-strom a druhý je optimalizovaný strom ze sekce \ref{anal.improvement.orderby.storeindex}, který seskupuje totožné prvky do pole místo aby je vložil do vrcholu.
...
Třída představuje optimalizovaný $(a, b)$-strom ze sekce analýzy \ref{anal.improvement.orderby.storeindex}.
...
Řešení je obsaženo v třídě \texttt{ABTreeStreamedSorter<T>} a odpovídá řešení ze sekce analýzy \ref{anal.improvement.orderby.streamed}.
...
\item Implementaci metody \texttt{Process(Element[] result, int matcherID)}.
\\*V~momentě nalezení výsledku vlákno vypočte hodnotu výrazu prvního klíče třídění \texttt{firstKeyExpression} pomocí \texttt{result} a určí přihrádku voláním metody \texttt{Hash} s vypočtenou hodnotou výrazu.
...
V původním řešení jsme použili při slévání výsledků do paralelní hešovací tabulky strukturu \texttt{GroupDictKey} jako klíč seskupení.
...
Porovnání v hešovací tabulce vyvolá přístup k dané položce a ne hodnotám v tabulce.
Pokud položka neexistuje v hešovací tabulce, tak je pole \texttt{temporaryRow} překopírováno do tabulky voláním metody \texttt{resTable.StoreTemporaryRow()}.
V opačném případě je položka nastavena na \texttt{null}. To odpovídá návrhu ze sekce analýzy \ref{anal.improvement.groupby}. 
...
Pro \texttt{Dictionary} existuje ekvivalent s jedním záznamem \texttt{HashSet}, ale protože již používáme původní paralelní hešovací tabulku, tak využijeme i \texttt{Dictionary}. 
...
Hodnota klíče je uložena v položce \texttt{T~aggResult}.
Definuje:
\begin{itemize}
...
Metoda se používá k získání heše výsledku prohledávání při vkládání do hešovací tabulky.
\end{itemize}

Při vkládání pole do hešovací tabulky musíme prvky porovnat. 
...
Třída vytváří pole \texttt{AggregateBucketResult[]}, které se vkládá do hešovací tabulky.
Obsahuje:
\begin{itemize}
...
\item Položku \texttt{bool lastWasInserted}, která říká, zda poslední vytvořené pole bylo vloženo do hešovací tabulky.
...
\item Položku \texttt{BucketKeyFactory[] matcherBucketFactories}, kde každý záznam odpovídá třídě vytvářející pole vkládané do hešovací tabulky jednoho vlákna.
...
\item Implementaci metody \texttt{Process(Element[] result, int matcherID)}.\\*
Vlákno volající metodu vytvoří vkládané pole pomocí svého objektu \\*\texttt{matcherBucketFactories[matcherID]} a vloží jej do hešovací tabulky.
\end{itemize}
...
\section{Překlad a spuštění dotazovacího enginu}
...
Uživatel zadá dotaz pomocí jazyka PGQL ze sekce \ref{req.pgql}.
